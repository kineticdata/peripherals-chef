...
class ChefRetrieveCookbooksV1
  ...
  def execute
    ...
    request_url = @info_values["organization_endpoint"] + "/cookbooks"
    headers = []
    payload = ""
    response = build_chef_request("GET",request_url,headers,payload,username,private_key)

    json = JSON.parse(response.body)
    return "<results><result name=\"Cookbook Names\">#{json.keys.sort.join(",")}</result></results>"
