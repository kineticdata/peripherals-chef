...
class ChefRetrieveRunlistV1
  ...
  def execute
    ...
    headers = []
    payload = ""
    response = chef_request("GET",request_url,headers,payload,username,private_key)
    ...
    puts json
    run_list = json["run_list"].join(",")
    puts "Returned results run_list:" + run_list if @debug_logging_enabled
    return "<results><result name=\"Run List\">#{escape(run_list)}</result></results>"
  end
