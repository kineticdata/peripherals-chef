...
class ChefCookbookRetrieveV1
  ...
  def execute
    ...
    request_url = @info_values["organization_endpoint"] + "/cookbooks/" + @parameters["cookbook_name"]
    headers = []
    payload = ""
    response = build_chef_request("GET",request_url,headers,payload,username,private_key)

    json = JSON.parse(response.body)
    cookbook = json[@parameters["cookbook_name"]]
    return "<results><result name=\"Cookbook JSON\">#{escape(cookbook.to_json)}</result></results>"
  end
