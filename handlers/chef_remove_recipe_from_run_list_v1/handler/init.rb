...
class ChefRemoveRecipeFromRunListV1
  ...
  def execute
    ...
    headers = []
    payload = ""
    response = build_chef_request("GET",request_url,headers,payload,username,private_key)
    ...
    runList = jsonOutput["run_list"]
    runList.delete_if {|x| x == "recipe[" + @parameters["recipe_name"] + "]"}
    jsonOutput["run_list"]= runList

    post_url = @info_values["organization_endpoint"] + "/nodes/" + @parameters["node_name"]
    postPayload = jsonOutput.to_json
    response = build_chef_request("PUT",request_url,headers,postPayload,username,private_key)

    puts response
    return "<results name='nodeJson'>{response}</results>"
  end
  ...
  def build_chef_request(http_method, url, headers, payload, username, private_key)
    uri = URI(url)
