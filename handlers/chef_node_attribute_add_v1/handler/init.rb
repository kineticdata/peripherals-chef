...
class ChefNodeAttributeAddV1
  ...
  def execute
    ...
    # Retrieve the existing Chef node along with all its current attributes
    request_url = "#{@info_values['organization_endpoint']}/nodes/#{@parameters['node_name']}"
    response = chef_request("GET",request_url,[],"",username,private_key)

    chef_error(response) if response.code != 200

    # If no error, rebuild the node object with the updated attribute and PUT it to the Chef server

    # Unset the Create Id so that JSON doesn't try to parse as a Chef::Node and instead returns it
    # as a normal JSON hash
    JSON.create_id = nil
    node = JSON.parse(response.body)
    node_attributes = node["normal"]

    # Add the new attribute to the node
    node_attributes[@parameters['attribute_name']] = @parameters['attribute_value']
    node["normal"] = node_attributes

    # Make the PUT request to the Chef Server
    response = chef_request("PUT",request_url,[],node.to_json,username,private_key)
