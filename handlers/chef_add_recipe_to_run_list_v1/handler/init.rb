...
class ChefAddRecipeToRunListV1
  ...
  def execute
    ...
    runList = json["run_list"]
    recipeString = "recipe[" + @parameters["recipe_name"] + "]"
    runList.push(recipeString)
    json["run_list"] = runList

    payload = json.to_json
    response = build_chef_request("PUT",request_url,headers,payload,username,private_key)
    puts response
    return "<results><result name='node_json'>#{response}</result></results>)"
